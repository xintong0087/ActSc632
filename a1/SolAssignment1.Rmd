...
sev.phi<-summary(model.sev)$dispersion

with(model.sev, cbind(res.deviance = deviance/sev.phi, 
                      df = df.residual, 
                      p = pchisq(deviance/sev.phi, df.residual, lower.tail = FALSE)))
```

The severity model is a good fit. We do not have sufficient evidence to reject the hypothesis based on the residual deviance, given by $171.9565$ on $164$ degrees of freedom, as its corresponding p-value is $0.3195037$.
...
print(c(model.freq$deviance, model.freq.new$deviance, 
      model.freq.new$df.residual - model.freq$df.residual))
pchisq(model.freq$deviance - model.freq.new$deviance,
       model.freq.new$df.residual - model.freq$df.residual, 
       lower.tail=FALSE)
```
...
sev.phi.new <- summary(model.sev.new)$dispersion

cbind(res.deviance = model.sev.new$deviance,
      df = model.sev.new$df.residual,
      p = pchisq(model.sev.new$deviance/sev.phi.new, model.sev.new$df.residual, lower.tail = FALSE))

print(c(model.sev$deviance, model.sev.new$deviance, 
      model.sev.new$df.residual - model.sev$df.residual))
cbind(diff.deviance = (model.sev.new$deviance - model.sev$deviance)/sev.phi.new ,
      diff.df = model.sev.new$df.residual - model.sev$df.residual,
      p = pchisq((model.sev.new$deviance - model.sev$deviance)/sev.phi.new,
                 model.sev.new$df.residual - model.sev$df.residual,
                 lower.tail = FALSE))
```

For the severity however, the LRT statistic is given by $130.72$ on $124$ degrees of freedom, 
with corresponding p-value given by $0.3221792$. 
Hence in this case, including age and sex does not provide a model that has a better fit. 
